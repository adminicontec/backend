...
const StoreCourseSchema = new Schema({
  ...
  collection: 'store_courses', timestamps: { createdAt: 'created_at', updatedAt: 'updated_at' }
});
...
StoreCourseSchema.plugin(mongoose_delete, {
  deletedAt: true,
  overrideMethods: 'all',
  indexFields: 'all'
});
...
// StoreCourseSchema.methods.invalid_fields = ["field1", "field2"];

// INFO: Si desea implementar un metodo despues de guardar habilite el siguiente metodo
// StoreCourseSchema.methods.postSave = (parameters) => {
// }


export const StoreCourseModel = mongoose.model<any, any>('StoreCourse', StoreCourseSchema);
