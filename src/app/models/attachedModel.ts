...
const AttachedSchema = new Schema({
  // @add_schema Add schema here
  category: {
    type: Schema.Types.ObjectId,
    ref: 'AttachedCategory',
    required: true
  },
  files: [{
    url: {
  ...
  collection: 'attacheds' ,timestamps: { createdAt: 'created_at', updatedAt: 'updated_at' }
});
...
AttachedSchema.plugin(mongoose_delete,{
  deletedAt : true,
  overrideMethods: 'all',
  indexFields: 'all'
});
...
// AttachedSchema.methods.invalid_fields = ["field1", "field2"];

// INFO: Si desea implementar un metodo despues de guardar habilite el siguiente metodo
// AttachedSchema.methods.postSave = (parameters) => {
// }


export const AttachedModel = mongoose.model<any, any>('Attached', AttachedSchema);
