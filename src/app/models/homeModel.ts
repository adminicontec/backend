...
const HomeSchema = new Schema({
  ...
  collection: 'homes' ,timestamps: { createdAt: 'created_at', updatedAt: 'updated_at' }
});
...
HomeSchema.plugin(mongoose_delete,{
  deletedAt : true,
  overrideMethods: 'all',
  indexFields: 'all'
});
...
// HomeSchema.methods.invalid_fields = ["field1", "field2"];

// INFO: Si desea implementar un metodo despues de guardar habilite el siguiente metodo
// HomeSchema.methods.postSave = (parameters) => {
// }


export const HomeModel = mongoose.model<any, any>('Home', HomeSchema);
