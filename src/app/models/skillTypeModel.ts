...
const SkillTypeSchema = new Schema({
  ...
  collection: 'skill_types' ,timestamps: { createdAt: 'created_at', updatedAt: 'updated_at' }
});
...
SkillTypeSchema.plugin(mongoose_delete,{
  deletedAt : true,
  overrideMethods: 'all',
  indexFields: 'all'
});
...
// SkillTypeSchema.methods.invalid_fields = ["field1", "field2"];

// INFO: Si desea implementar un metodo despues de guardar habilite el siguiente metodo
// SkillTypeSchema.methods.postSave = (parameters) => {
// }


export const SkillTypeModel = mongoose.model('SkillType', SkillTypeSchema);
