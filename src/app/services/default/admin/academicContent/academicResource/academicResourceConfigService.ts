...
class AcademicResourceConfigService {
  ...
  public insertOrUpdate = async (params: IAcademicResourceConfig) => {
    try {
      ...
      if (params?.config?.course_modes) {
        const isValidObjectId = await ObjectID.isValid(params?.config?.course_modes)
        if (!isValidObjectId) {
          params.config.course_modes_mixed = params?.config?.course_modes;
          params.config.course_modes = undefined
        } else {
          params.config.course_modes_mixed = undefined
        }
      }
