...
class ModularService {
  ...
  public insertOrUpdate = async (params: IModular) => {
    ...
    try {
      if (params.id) {
        const register = await Modular.findOne({_id: params.id})
        if (!register) return responseUtility.buildResponseFailed('json', null, {error_key: 'modular.not_found'})
        ...
        const exist = await Modular.findOne({ name: params.name })
        if (exist) return responseUtility.buildResponseFailed('json', null, { error_key: {key: 'modular.insertOrUpdate.already_exists', params: {name: params.name}} })

        const response: any = await Modular.create(params)
   ...
   * @returns
   */
  public delete = async (params: IModularDelete) => {
    try {
      const find: any = await Modular.findOne({ _id: params.id })
      if (!find) return responseUtility.buildResponseFailed('json', null, { error_key: 'modular.not_found' })
   ...
   * @returns
   */
  public list = async (filters: IModularQuery = {}) => {
    ...
    let registers = []
    try {
      registers =  await Modular.find(where)
      .select(select)
    ...
    return responseUtility.buildResponseSuccess('json', null, {
      additional_parameters: {
        ...
        total_register: (paging) ? await Modular.find(where).count() : 0,
...
export { ModularService as DefaultAdminModularModularService };
