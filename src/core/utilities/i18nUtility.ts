...
// @import types
import {IAddResourceBundle} from "@scnode_core/types/default/i18n/i18nTypes"
// @end
class I18nUtility {
  ...
  public buildI18nDirectories = async () => {
    ...
    for (const key in this.langs) {
      if (this.langs.hasOwnProperty(key)) {
        ...
        const dest = full_dir_app + key + '.json';
        try {
          const result = await jsonMerger.mergeFiles(element,{stringify: true});
