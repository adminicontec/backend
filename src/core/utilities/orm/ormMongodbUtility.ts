  ...
  public configDb = async (connection_data: MongoConnectionData) => {
    ...
    if (connection_data.hasOwnProperty('host') && connection_data.hasOwnProperty('dbname')) {
      try {
        ...
        const defaultUrl = `mongodb://${connection_data['host']}/${connection_data['dbname']}${(connection_data.options) ? connection_data.options : ''}`;
        const mongoUrl = connection_data?.uri?.length ? connection_data?.uri : defaultUrl
